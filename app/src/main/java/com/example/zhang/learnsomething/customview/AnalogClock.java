...
//时要进行适当的缩放。
    ...
    public AnalogClock(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mHourHand = ContextCompat.getDrawable(context,R.mipmap.clock_hand_hour);
        mMinuteHand = ContextCompat.getDrawable(context,R.mipmap.clock_hand_minute);
        mDial = ContextCompat.getDrawable(context,R.mipmap.clock_dial);
        mTime =new Date(SystemClock.currentThreadTimeMillis());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode != MeasureSpec.UNSPECIFIED && widthSize < mDialWidth) {
            hSale = (float) widthSize / (float) mDialWidth;
        }
        if (heightMode != MeasureSpec.UNSPECIFIED && heightSize < mDialHeight) {
            vSale = (float )heightSize / (float) mDialHeight;
        }
        float scale = Math.min(hSale, vSale);
    ...
    private void onTimeChanged() {
        ...
        int minute = mTime.getMinutes();
        int second = mTime.getSeconds();
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(Intent.ACTION_TIMEZONE_CHANGED)) {
                ...
                mTime = new Date(TimeZone.getTimeZone(tz).getID());
            }
            //进行时间的更新
            onTimeChanged();
